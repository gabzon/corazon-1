<div>
    @if (! auth()->user()->isRegisteredInEvent($event))

    <form action="{{ route('event.register') }}" method="POST" class="mr-3">
        @csrf
        <input type="hidden" name="registrable_type" value="{{ get_class($event) }}" />
    ...
    <div class="inline-flex items-center">
        ...
        <div class="mr-2">
            <livewire:profile.user-registration-status-badge :model="$event" size="large" />
        </div>

        <form action="{{ route('event.unregister') }}" method="POST">
            @csrf
            @method('DELETE')
            <input type="hidden" name="registrable_type" value="{{ get_class($event) }}" />
            <input type="hidden" name="id" value="{{ $event->id }}" />
